<template>
  <el-container>
    <el-header class="cate_mana_header">
      <el-input
        placeholder="请输入栏目名称"
        v-model="content" style="width: 200px;">
      </el-input>
  ...
  export default{
    methods: {
      addNewCate(){
        this.loading = true;
        var _this = this;
        postRequest('/admin/queryAllTitles/', {content: this.content}).then(resp=> {
          if (resp.status == 200) {
            var json = resp.data;
      ...
      handleEdit(index, row){
        ...
        this.$prompt('请输入新名称', '编辑', {
          ...
          if (value == null || value.length == 0) {
            ...
            _this.loading = true;
            putRequest("/admin/category/", {id: row.id, content: value}).then(resp=> {
              var json = resp.data;
              _this.$message({
                type: json.status,
                message: json.msg
      ...
      refresh(){
        let _this = this;
        getRequest("/admin/queryAllTitles").then(resp=> {
