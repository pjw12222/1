...
export default {
  mounted: function() {
    ...
    if (from != null && from != "" && from != undefined) {
      ...
      this.id = id;
      this.loading = true;
      getRequest("/community/queryInfoHeadById/" + id).then(
        resp => {
          _this.loading = false;
          if (resp.status == 200) {
            let middleresult = {};
            ...
            middleresult.mdContent = data.headContent;
            middleresult.content = data.scope1;
  ...
  methods: {
    ...
    saveBlog(state) {
      ...
      var _this = this;
      _this.loading = true;
      postRequest("/community/insertInfoHead", {
        id: _this.id,
        userId: _this.userId,
        title: _this.article.title,
        headContent: _this.article.mdContent,
        scope1: _this.article.content
      }).then(
        resp => {
          _this.loading = false;
          if (resp.status == 200) {
            ...
            //            }
            if (state == 1) {
              _this.$router.replace({ path: "/shareList" });
            }
          }
        },
        resp => {
